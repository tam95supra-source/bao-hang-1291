...
async function block(code: string, detail: string, fields: Record<string, unknown> = {}) {
  await setCursor({ state: "BLOCKED", last_error_at: new Date().toISOString(), last_error_code: code, last_error_detail: detail.slice(0, 500), ...fields });
  await sheet({ mode: "recovery_state", state: "RECOVERY_BLOCKED" });
}
...
Deno.serve(async (req) => {
  try {
    ...
    const { data: cursorRow, error: cursorError } = await admin.from("sheet_recovery_cursor")
      .select("imported_sequence,acknowledged_sequence,state").eq("singleton", true).single();
    if (cursorError) throw cursorError;
    if (!cursorRow) throw new Error("RECOVERY_CURSOR_MISSING");
    const cursor = cursorRow as { imported_sequence?: number; acknowledged_sequence?: number; state?: string };
    if (cursor.state === "BLOCKED") {
      await sheet({ mode: "recovery_state", state: "RECOVERY_BLOCKED" });
      return json({ ok: false, blocked: true, cursor }, 409);
    }
    ...
    for (const event of events) {
      ...
      if (!Number.isFinite(sequence) || sequence !== expected) {
        await block("SHEET_SEQUENCE_GAP", `expected=${expected},got=${sequence}`, { imported_sequence: lastImported });
        return json({ ok: false, blocked: true, error: "SHEET_SEQUENCE_GAP", expected, got: sequence }, 409);
      }
      const { data, error: importError } = await admin.rpc("import_authority_event_service", { p_event: event });
      if (importError) {
        await block("IMPORT_RPC_ERROR", importError.message || "RPC_ERROR", { imported_sequence: lastImported });
        return json({ ok: false, blocked: true, error: "IMPORT_RPC_ERROR", sequence }, 409);
      }
      if (data?.ok !== true) {
        const errorCode = String(data?.error_code ?? "IMPORT_BLOCKED");
        await block(errorCode, String(data?.error ?? "blocked"), { imported_sequence: lastImported });
        return json({ ok: false, blocked: true, sequence, event_id: event.event_id, error_code: errorCode }, 409);
      }
      lastImported = sequence;
      expected++;
      acks.push({
        event_id: String(event.event_id), sheet_sequence: sequence,
        accepted_at_sheet: String(event.sheet_ack_at ?? event.accepted_at_authority ?? ""),
        service_ack_at: new Date().toISOString(), reconciliation_status: "SERVICE_ACKED",
