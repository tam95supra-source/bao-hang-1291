...
-- Operator OPEN state is delivered by realtime marker; do not create suppressed FCM notifications.
CREATE OR REPLACE FUNCTION public.report_shortage_rpc(p_sku text, p_client_request_id text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$ declare u uuid:=public.app_uid(); r public.user_role; v jsonb; issue jsonb; begin if u is null then raise exception 'AUTH_REQUIRED'; end if; select role into r from public.profiles where id=u and active=true; if r not in ('PICKER','ADMIN') then raise exception 'FORBIDDEN'; end if; v:=public.report_shortage_atomic(p_sku,u,p_client_request_id); issue:=v->'issue'; return jsonb_build_object('issue',issue-'report_count'-'latest_reporter_name'-'assigned_name'-'assigned_id','already_reported',coalesce((v->>'already_reported')::boolean,false),'duplicate_request',coalesce((v->>'duplicate_request')::boolean,false),'message',coalesce(v->>'message','Đã ghi nhận báo thiếu')); end $function$;
