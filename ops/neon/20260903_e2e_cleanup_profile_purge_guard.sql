-- BÁO HÀNG 1291 — keep exact E2E cleanup compatible with guarded profile deletion.
-- The bypass is transaction-local and only reachable inside the exact, marker-validated E2E cleanup RPC.
...
DECLARE
  ...
  r_sku bigint; r_profile bigint; r_report bigint; r_owner bigint; r_event bigint; r_notification bigint; r_realtime bigint; real_ok boolean;
BEGIN
  ...
  IF seed_mode THEN
    ...
    RETURN jsonb_build_object('ok',true,'mode','seed','profiles_seeded',4,'skus_seeded',2,'real_user_10060_preserved',true);
  END IF;
  ...
  IF inventory_mode THEN
    RETURN jsonb_build_object(
      ...
      'real_user_10060_preserved',true
    );
  END IF;
  ...
  RETURN jsonb_build_object('ok',true,'mode','guard','real_user_10060_preserved',true);
END
$function$
;
