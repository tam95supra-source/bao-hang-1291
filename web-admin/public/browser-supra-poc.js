(() => {
  ...
  const OPEN_LINK_ID = 'browserSupraOpen';
  ...
  function prepareDirectNavigation(event) {
    const link = event.currentTarget;
    link.href = `${SUPRA_ORIGIN}?bao_hang_1291_browser_nav=${Date.now()}`;
    setMessage(
      NAV_MESSAGE_ID,
      'Đang mở Supra bằng link điều hướng trực tiếp. Cách này không dùng window.open, không phụ thuộc CORS và không cần cho phép popup. Nếu tab mới hiện bất kỳ HTTP/page nào thì đường browser→Supra tồn tại; nếu hiện ERR_CONNECTION_RESET/không thể truy cập thì browser path cũng bị chặn. Không dùng credential.',
      'info',
    );
  }
  ...
  function mount() {
    ...
    card.innerHTML = `
      ...
      <p class="muted">Chẩn đoán 2 lớp, không dùng credential: (1) fetch no-cors có bắt CSP violation; (2) top-level navigation bằng link trực tiếp ở tab mới để loại CORS và popup script hoàn toàn.</p>
      <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
        <button id="${BUTTON_ID}" class="secondary" type="button">THỬ FETCH TỪ TRÌNH DUYỆT</button>
        <a id="${OPEN_LINK_ID}" class="secondary" href="${SUPRA_ORIGIN}" target="_blank" rel="noopener noreferrer" style="display:inline-flex;align-items:center;text-decoration:none">MỞ SUPRA Ở TAB MỚI</a>
      </div>
    ...
    document.getElementById(OPEN_LINK_ID)?.addEventListener('click', prepareDirectNavigation);
  }
