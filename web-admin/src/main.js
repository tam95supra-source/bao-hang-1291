...
function formatTime(value) {
  ...
  return Number.isNaN(d.getTime()) ? escapeHtml(value) : d.toLocaleString(getLocale(), { hour12: false });
}
...
async function renderLogs() {
  $('#content').innerHTML = `<div class="heading"><div><p class="eyebrow">CHẨN ĐOÁN</p><h2>Nhật ký hệ thống</h2></div><div class="actions"><button id="createFullLog" class="primary">TẠO FILE LOG ĐẦY ĐỦ</button><button id="refreshLogs" class="secondary">Làm mới</button></div></div>
    ...
    <article class="card"><strong>Lưu trữ log chẩn đoán</strong><p>File log Web và thiết bị được lưu trực tiếp trong thư mục Google Drive của Báo hàng 1291, không lưu file hoặc metadata log mới trên service. Log tự động chỉ tạo khi có lỗi đáng chú ý và được gộp theo chu kỳ để tránh spam.</p><p class="muted">Nút “Tạo file log đầy đủ” tạo một ảnh chụp chẩn đoán của phiên Web hiện tại, lưu lên Google Drive và tải file về máy.</p></article>
...
async function downloadWebLog(id){
  try{
    setBusy(true,'Đang tải log web…');
    const r=await api('download-web-log',{id});
    if(!r.gzip_base64)throw new Error('Google Drive không trả dữ liệu log web.');
    const raw=atob(r.gzip_base64),bytes=new Uint8Array(raw.length);
    for(let i=0;i<raw.length;i++)bytes[i]=raw.charCodeAt(i);
    const url=URL.createObjectURL(new Blob([bytes],{type:'application/gzip'}));
    const link=document.createElement('a');
    link.href=url;link.download=r.file_name||`bao-hang-1291-web-log-${id}.jsonl.gz`;
    document.body.appendChild(link);link.click();link.remove();
    setTimeout(()=>URL.revokeObjectURL(url),1000);
  }catch(error){alert(safeMessage(error));}
  finally{setBusy(false);}
...
async function renderSla(){
 $('#content').innerHTML=`<div class="heading"><div><p class="eyebrow">THỜI GIAN</p><h2>Thời gian nghiệp vụ</h2></div></div><div id="slaBody"></div>`;try{const c=await api('get-operational-config');$('#slaBody').innerHTML=`<article class="card"><div class="form-grid"><label>Thời gian tiếp nhận (phút)<input id="ackMin" type="number" min="1" max="480" value="${c.acknowledge_minutes}"><small>Từ lúc người lấy hàng báo thiếu đến khi người xử lý nhận yêu cầu.</small></label><label>Nhắc lại nếu chưa xử lý (phút)<input id="reminderMin" type="number" min="1" max="480" value="${c.reminder_minutes}"><small>Khoảng cách giữa các lần nhắc khi yêu cầu vẫn chưa hoàn tất.</small></label><label>Thời gian xử lý sau khi nhận (phút)<input id="replenishMin" type="number" min="1" max="480" value="${c.replenish_minutes}"><small>Thời gian theo dõi sau khi người xử lý đã nhận yêu cầu.</small></label><label>Nhắc người lấy hàng xác nhận (phút)<input id="pickerAckMin" type="number" min="1" max="60" value="${c.picker_ack_reminder_minutes}"><small>Dùng cho cảnh báo có hàng hoặc được phép bỏ qua.</small></label><label>Nhắc lại khi đã tìm thấy hàng (phút)<input id="foundItemReminderMin" type="number" min="1" max="60" value="${c.found_item_reminder_minutes||5}"><small>Mặc định 5 phút. Cảnh báo sẽ nhắc lại tới khi người lấy hàng xác nhận.</small></label><label class="check"><input id="autoSkipEnabled" type="checkbox" ${c.auto_skip_enabled?'checked':''}> Tự động cho phép bỏ qua khi quá thời gian</label><label>Tự động bỏ qua sau (phút)<input id="autoSkipAfter" type="number" min="15" max="4320" value="${c.auto_skip_after_minutes||120}"><small>Có thể tắt hoàn toàn.</small></label></div><button id="saveSla" class="primary">LƯU THỜI GIAN NGHIỆP VỤ</button><div id="slaMsg" class="message" hidden></div></article>`;$('#saveSla').onclick=async()=>{try{await api('save-operational-config',{acknowledge_minutes:Number($('#ackMin').value),reminder_minutes:Number($('#reminderMin').value),replenish_minutes:Number($('#replenishMin').value),picker_ack_reminder_minutes:Number($('#pickerAckMin').value),found_item_reminder_minutes:Number($('#foundItemReminderMin').value),auto_skip_enabled:$('#autoSkipEnabled').checked,auto_skip_after_minutes:Number($('#autoSkipAfter').value)});message('#slaMsg','Đã lưu thời gian nghiệp vụ.','good');}catch(e){message('#slaMsg',safeMessage(e),'error');}};}catch(e){$('#slaBody').innerHTML=`<div class="message" data-type="error">${escapeHtml(safeMessage(e))}</div>`;}
}
